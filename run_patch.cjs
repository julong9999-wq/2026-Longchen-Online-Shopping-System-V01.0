                                     ...
                                     <table className="w-full text-sm text-left border">
                                         ...
                                         <tbody className="divide-y divide-slate-100">
                                             {Object.entries(activeExpenses.reduce((acc, exp) => {
                                                 ...
                                                 <tr key={ym} className="hover:bg-slate-50"><td className="p-2 text-slate-600 font-mono">{ym}</td><td className="p-2 text-right font-bold text-slate-800">{formatCurrency(total)}</td></tr>
                                             ))}
                                         </tbody>
                                     </table>
                                 </div>
                                     ...
                                     <table className="w-full text-sm text-left border">
                                         ...
                                         <tbody className="divide-y divide-slate-100">
                                             {Object.entries(activeExpenses.reduce((acc, exp) => {
                                                 acc[exp.category] = (acc[exp.category] || 0) + exp.amount;
                                                 return acc;
                                             }, {})).sort((a,b)=>b[1]-a[1]).map(([cat, total], idx) => (
                                                 <tr key={cat} className="hover:bg-slate-50"><td className="p-2 text-slate-600"><span className="inline-block w-2.5 h-2.5 rounded-full mr-2" style={{backgroundColor: ['#f97316', '#3b82f6', '#10b981', '#f43f5e', '#ec4899', '#6366f1'][idx % 6]}}></span>{cat}</td><td className="p-2 text-right font-bold text-slate-800">{formatCurrency(total)}</td></tr>
                                             ))}
                                         </tbody>
                                     </table>
                                 </div>
                                     ...
                                     <table className="w-full text-sm text-left border">
                                         ...
                                         <tbody className="divide-y divide-slate-100">
                                             {Object.entries(activeExpenses.reduce((acc, exp) => {
                                                 ...
                                                 <tr key={payer} className="hover:bg-slate-50"><td className="p-2 text-slate-600 font-bold">{payer}</td><td className="p-2 text-right font-bold text-slate-800">{formatCurrency(total)}</td></tr>
                                             ))}
                                         </tbody>
                                     </table>
                                 </div>
...
if (txt.indexOf(targetEnd) === -1) {
   console.log('Cant find targetEnd');
} else {
