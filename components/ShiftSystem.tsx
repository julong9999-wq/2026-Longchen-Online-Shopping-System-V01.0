...
const getShiftColor = (start: string, end: string) => {
                    ...
                    <div className="p-2 flex-1 overflow-y-auto flex flex-col gap-1.5">
                            ...
                            <div className="flex flex-col gap-2 w-full">
                                ...
                                <div className="border-t border-slate-100 pt-2 flex justify-end">
                                    <button 
                                        ...
                                        className={`px-3 h-8 rounded-lg font-bold text-white shadow-sm flex items-center gap-1 text-xs ${mainColorClass}`}
                                    >
                                        <Plus size={14}/> 新增排班
                                    </button>
                                </div>
                        ...
                        <div className="mt-1">
                                            ...
                                            {(() => {
                                                ...
                                                return (
                                                    <>
                                                        ...
                                                        <span className={`font-mono font-bold shrink-0 ${getShiftColor(r.startTime, r.endTime)}`}>{r.startTime}-{r.endTime} ({Number(hrs.toFixed(2))}H)</span>
                                                    </>
                                                );
                                            })()}
                            ...
                            {upcomingSummary.length > 0 && (
                                <div className="mt-2 bg-slate-50 p-1.5 rounded-lg border border-slate-200 text-xs shadow-inner">
                                                ...
                                                <div className="flex-1 flex flex-col p-px md:p-0.5 gap-px">
                                                    {dayRecs.map(r => {
                                                        ...
                                                        <div key={r.id} className={`${bClass} border flex flex-col items-center justify-around flex-1 min-h-[0] rounded-sm p-0.5 overflow-hidden`} title={`${currentLocs.find(l=>l.id===r.locationId)?.name}`}>
                                                            ...
                                                            <div className={`text-[9px] md:text-xs font-bold leading-none ${getShiftColor(r.startTime, r.endTime)}`}>{r.startTime}</div>
                                                            <div className={`text-[9px] md:text-xs font-bold leading-none ${getShiftColor(r.startTime, r.endTime)}`}>{r.endTime}</div>
                                                        </div>
                                                    )})}
                                                </div>
                                            </div>
                    ...
                    <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md shadow-xl flex flex-col overflow-hidden animate-slide-up">
                        ...
                        <div className="p-4 flex flex-col gap-3">
                            ...
                            <div className="flex gap-2 items-center">
                                <input type="time" step="1800" className={`flex-1 h-10 px-3 border rounded-lg bg-slate-50 font-mono text-sm font-bold ${getShiftColor(planStartTime, planEndTime)}`} required value={planStartTime} onChange={e => setPlanStartTime(e.target.value)} />
                                <span className="text-slate-400">~</span>
                                <input type="time" step="1800" className={`flex-1 h-10 px-3 border rounded-lg bg-slate-50 font-mono text-sm font-bold ${getShiftColor(planStartTime, planEndTime)}`} required value={planEndTime} onChange={e => setPlanEndTime(e.target.value)} />
                            </div>
                            <div>
