         ...
         <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 space-y-4">
               ...
               {isPaymentExpanded && (
                  <div className="p-3 bg-white border border-slate-200 rounded-xl space-y-3 mt-1">
                     <div className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2 items-center">
                        <input type="text" placeholder="訂單序號" value={paymentForm.orderNo} onChange={e => setPaymentForm({...paymentForm, orderNo: e.target.value})} className="border border-slate-300 rounded-lg p-2 text-sm font-mono min-w-0" />
                        <input type="text" placeholder="名稱" value={paymentForm.name} onChange={e => setPaymentForm({...paymentForm, name: e.target.value})} className="border border-slate-300 rounded-lg p-2 text-sm min-w-0" />
                        <input type="number" step="any" placeholder="金額" value={paymentForm.amount} onChange={e => setPaymentForm({...paymentForm, amount: e.target.value})} className="border border-slate-300 rounded-lg p-2 text-sm font-mono min-w-0" onKeyDown={e => e.key === 'Enter' && handleAddPayment()} />
                        <button onClick={handleAddPayment} className="bg-orange-500 text-white rounded-lg p-2 font-bold hover:bg-orange-600 transition-colors shrink-0">
                           <Plus size={20} />
                        </button>
                     </div>
                     <SubTable 
            ...
            <div className="flex flex-col gap-1.5">
               ...
               {isCollectionExpanded && (
                  <div className="p-3 bg-white border border-slate-200 rounded-xl space-y-3 mt-1">
                     <div className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2 items-center">
                        <input type="text" placeholder="訂單序號" value={collectionForm.orderNo} onChange={e => setCollectionForm({...collectionForm, orderNo: e.target.value})} className="border border-slate-300 rounded-lg p-2 text-sm font-mono min-w-0" />
                        <input type="text" placeholder="名稱" value={collectionForm.name} onChange={e => setCollectionForm({...collectionForm, name: e.target.value})} className="border border-slate-300 rounded-lg p-2 text-sm min-w-0" />
                        <input type="number" step="any" placeholder="金額" value={collectionForm.amount} onChange={e => setCollectionForm({...collectionForm, amount: e.target.value})} className="border border-slate-300 rounded-lg p-2 text-sm font-mono min-w-0" onKeyDown={e => e.key === 'Enter' && handleAddCollection()} />
                        <button onClick={handleAddCollection} className="bg-emerald-500 text-white rounded-lg p-2 font-bold hover:bg-emerald-600 transition-colors shrink-0">
                           <Plus size={20} />
                        </button>
                     </div>
                     <SubTable 
